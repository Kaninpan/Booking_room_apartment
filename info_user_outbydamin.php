...
<?php
     ...
     $query = "select * from user_info where Class = 'Out'";  
...
<div class="topnav">
  ...
  <a style="float:right" href="login.php?logout=<?php echo $id; ?>" onclick="return confirm('คุณต้องการออกจากระบบหรือไม่ ?');" class="btnbackid">ออกจากระบบ</a>
     ...
     <h1>ข้อมูลผู้เช่าที่ทำการย้ายออก</h1>
   <?php
			echo "<table  align='center' width='100%' border ='0'>";
			echo "<tr align='center' bgcolor='#00ad45'><td><h1>ชื่อ - นามสกุล</h1></td><td><h1>ห้องพัก</h1></td><td><h1>ที่อยู่</h1></td><td><h1>เบอร์โทรศัพท์</h1></td><td><h1>วันที่จอง</h1></td><td><h1>วันที่ย้ายออก</h1></td><td><h1>วันที่ทำเรื่องย้ายออก</h1></td></td>";
         $i=1;  
         if ($num = mysqli_num_rows($run)) {  
              while ($result = mysqli_fetch_assoc($run)) {  
                   echo "  
                        <tr bgcolor='#5ecc62'>   
                             <td style='font-size: 20px;' width='15%'><p>".$result['name']."</p></td>
                             <td style='font-size: 20px;' width='5%'><p><b>".$result['type']."</b></p></td>
                             <td style='font-size: 20px;' width='40%'><p>".$result['address']."</p></td>  
                             <td style='font-size: 20px;' width='10%'><p>".$result['TEL']."</p></td>
                             <td style='font-size: 20px;' width='8%'><p><b>".$result['reserve']."</b></p></td>
                             <td style='font-size: 20px;' width='7%'><p><b>".$result['mdy_out']."</b></p></td>
                             <td style='font-size: 20px;' width='13%'><p><b>".$result['notify_out']."</b></p></td>
                        </tr>
                   ";  
              }  
         } 
